...
const Hprop = (props) => {
    ...
    useEffect(() => {
        (async () => {
            const {data: {pcats, name, url}} = await getJson(parseUrl(props.json));
            setTitle(name);
            setPcats(pcats);
            setMore(url);
        })()
    ...
    return (
        <RadarWarp>
            <div className="radarColmn">
                <h2 className="title"><p>{title} </p> {more && <a onClick={() => trigger()}>更多 &gt;</a>}</h2>
                <div onClick={() => trigger()} className="radarUnitWarp">
                    {pcats.length > 0 && pcats.map((item) => (
                        <dl className="radarUnit" key={item.name}>
                            <dt><img alt={item.name} src={item.img}/></dt>
                            <dd>{item.name}</dd>
