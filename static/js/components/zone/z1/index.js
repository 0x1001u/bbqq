...
const Brandinfo1 = (props) => {
    ...
    useEffect(() => {
        (async () => {
            const {data: {title, frame, qrcode, tag1, tag2, slide}} = await getJson(parseUrl(props.json));
            let isLoadWx = false
            ...
            <div className="template-bg" style={{background: frame.bgcolor}}>
                <div className="brandinfo1-wrap">
                    <div className="brandinfo1-wrap_img-wrap">
                        <img className="brandinfo1-wrap_img-wrap_img" src={frame.logo} alt="logo"/>
                    </div>
                ...
                <div className="brandinfo1-footer"
                     style={{backgroundImage: 'url(' + frame.bgimg + ')'}}
                     onTouchStart={() => touchStart()}
                     onTouchEnd={() => touchEnd()}>
                    <div className="brandinfo1-footer_qrcode">
                        <img className="brandinfo1-footer_qrcode_img" src={qrcode.img} alt="qrcode"/>
                        <div className="brandinfo1-footer_qrcode_title">{qrcode.text}</div>
                    </div>
                </div>
            </div>
