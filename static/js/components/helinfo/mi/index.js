...
const Helinfo = (props) => {
  ...
  return visible ? (
    <RecordWarp onClick={() => setDialogVisible(!dialogVisible)}>
      ...
      <div className={normalVisible ? "right" : "hidden"}>
        <img
          alt={`能效${state.level}级`}
          src={`https://www.pzjdimg.com/bbqk/img/level${
            state.level ? state.level : 1
          }.png`}
        />
        <div>查看详情 &gt;</div>
      </div>
      <Dialog visble={dialogVisible} onClose={() => setDialogVisible(false)}>
                ...
                <thead>
                  <tr>
                    {titles.map((item, index) => (
                      <th key={item.k + index * 21}>{item.d}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {body.length > 0 &&
                    body.map((tritem, tridx) => {
                      return (
                        <tr key={tridx + 100}>
